...
const CpuandRamChart = () => {
  ...
  const generateCPUPricingData = () => {
    ...
    for (let i = 120; i >= 0; i--) { // 120 minutes (2 heures) avant l'heure actuelle
  ...
  <ResponsiveContainer width="100%" height={400}>
    <AreaChart data={cpuPricingData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
      <CartesianGrid strokeDasharray="3 3" />
      <XAxis dataKey="timestamp" type="number" domain={["dataMin", "dataMax"]} tickFormatter={(timestamp) => new Date(timestamp).toLocaleTimeString()} />
      <YAxis domain={[0, "auto"]} tickFormatter={(value) => `${value.toFixed(2)} €`} />
      <Tooltip labelFormatter={(value) => new Date(value).toLocaleTimeString()} formatter={(value) => `${value.toFixed(2)} €`} />
      <Legend />
...
<Box mb={4}>
  ...
  <ResponsiveContainer width="100%" height={400}>
    <AreaChart data={ramPricingData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
      <CartesianGrid strokeDasharray="3 3" />
      <XAxis dataKey="timestamp" type="number" domain={["dataMin", "dataMax"]} tickFormatter={(timestamp) => new Date(timestamp).toLocaleTimeString()} />
      <YAxis domain={[0, "auto"]} tickFormatter={(value) => `${value.toFixed(2)} €`} />
      <Tooltip labelFormatter={(value) => new Date(value).toLocaleTimeString()} formatter={(value) => `${value.toFixed(2)} €`} />
      <Legend />
      <Area type="monotone" dataKey="cost" stroke="#4caf50" fill="#4caf50" fillOpacity={0.3} />
    </AreaChart>
  </ResponsiveContainer>
</Box>
