...
const RamCostChart = () => {
            ...
            "http://98.66.205.79/api/v1/query_range",
            {
              params: {
                query:
                  ...
                  "sum by (pod) (container_memory_usage_bytes{image!=''})",
  ...
  const getTopPods = (data, n) => {
    const podAverages = data.map((pod) => ({
      ...pod,
      averageCpu:
        pod.data.reduce((sum, point) => sum + point.hourlyCost, 0) /
        pod.data.length,
    }));
  ...
  const formatDataForChart = (prometheusData) => {
    const podData = {};
    const cpuPricePerHour = 0.0165; // CPU price per hour in euros
    ...
    prometheusData.data.data.result.forEach((result) => {
      ...
      result.values.forEach((dataPoint) => {
        ...
        const cpuUsage = parseFloat(dataPoint[1]) / 1000000000;
        const hourlyCost = cpuUsage * cpuPricePerHour;

        if (!podData[podName]) {
          podData[podName] = [];
        }
        podData[podName].push({ timestamp, hourlyCost });
      });
    });
                ...
                <Box mb={4}>
                  ...
                  <ResponsiveContainer width="100%" height={400}>
                    ...
                    >
                      ...
                      <YAxis
                        domain={["auto", "auto"]}
                        tickFormatter={(value) => `${value.toFixed(2)}€`} // Display as euros
                        tick={{ fontSize: 12 }}
                      />
                      <Tooltip
                        ...
                        formatter={(value) => `${value.toFixed(2)}€`} // Display as euros
                      />
                      <Legend />
